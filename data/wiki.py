...
class load_WikiDataSetDGL(torch.utils.data.Dataset):
...
class WikiDatasetDGL(torch.utils.data.Dataset):
    ...
    def __init__(self, path,  name):
        ...
        self.train = load_WikiDataSetDGL(data_dir, name, split='train')
        self.test = load_WikiDataSetDGL(data_dir, name, split='test')
        self.val = load_WikiDataSetDGL(data_dir, name, split='val')
...
class WikiDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        with open(data_dir +"/" + name + "/" + name + '.pkl', "rb") as f:
            f = pickle.load(f)
            self.train = f[0]
            self.val = f[1]
            self.test = f[2]
