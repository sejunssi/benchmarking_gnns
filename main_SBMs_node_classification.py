...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs, train_soft_target=False):
    ...
    try:
        with tqdm(range(params['epochs'])) as t:
            ...
            with open(f"{params['seed']}_{str(net_params['residual'])}_{DATASET_NAME}_{MODEL_NAME}_ep{best_val_epoch}_acc_best_val.csv",'w') as f:
                if len(best_acc) == 3:
    ...
    with open(f'./{params["seed"]}_{str(net_params["residual"])}_{DATASET_NAME}_{MODEL_NAME}_test_result.csv', 'wt', newline='') as f:
