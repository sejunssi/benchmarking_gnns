...
def classic_smoothing(w, data, n_classes):
...
def make_label_smoothing(dataname, w, dataset, n_classes):
    ...
    for i, data in enumerate(dataset[0].dataset):
        dataset[0].dataset[i]['node_label'] = classic_smoothing(w, data['node_label'], n_classes)
        dataset[0].node_labels.append(classic_smoothing(w, data['node_label'], n_classes))
    w_str = str(w).split(".")[1]
...
def classic_label_smoothing(DataSetName, w_list):
...
classic_label_smoothing(DataSetName, w_list)
